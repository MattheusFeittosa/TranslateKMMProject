package com.plcoding.translator_kmm.voice_to_text.domain.presentation
...
class VoiceToTextViewModel(
    ...
    fun onEvent(event: VoiceToTextEvent) {
        when (event) {
            ...
            is VoiceToTextEvent.ToggleRecording -> toggleListener(event.languageCode)
            else -> Unit
        }
    }

    private fun toggleListener(languageCode: String) {
        parser.cancel()
        if(state.value.displayState==DisplayState.SPEAKING){
            parser.stopListener()
        }else{
            parser.startListener(languageCode)
        }
    }
}